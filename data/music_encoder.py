...
if __name__ == '__main__':
    ...
    # ARGSSSSS
    # Namespace(input_folder='./output_uncondition', output_folder='./Output_Uncondtitionl_MIDI',
    #           encode_official_maestro=False, mode='to_midi', stretch_factors='0.95,0.975,1.0,1.02
    # 5, 1.05
    # ', pitch_transpose_lower=-3, pitch_transpose_upper=3)
    ...
    #     print("FFFFFFFFFFFFFFFFFFFFFFFFFF", path)
    #     print("OUT FFFFFFFFFFFFFFFFFFFFFF", out_dir)
    #     filename, extension = os.path.splitext(os.path.basename(path))
    ...
    if args.mode == 'to_txt' or args.mode == 'midi_to_npy':
        ...
        convert_f = defs.run_from_text if args.mode == 'to_midi' else run_npy_to_midi
        ...
        with mpl.Pool(num_cpus - 1) as pool:
            pool.map(functools.partial(convert_f, out_dir=args.output_folder), input_paths)
        midi_files= list(find_files_by_extensions('.\Output_Uncondtitionl_MIDI', ['.mid']))
        # print (type(midi_files[0]))
        print('Test converted! Spent {}s to convert {} samples.'.format(time.time() - start, len(input_paths)))
        # done? write "1" on ../flask-server/DidGenerate.txt        
        to_know = os.path.exists("./IsParsed.txt")
        f1 = open("./IsParsed.txt", 'w')        
        f1.write("1")        
        print ('Se dio correctamente todo')
        f1.close()
    else:
